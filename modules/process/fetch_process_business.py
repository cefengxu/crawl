...
class FetchProcessBusiness(BaseProcess):
    def __init__(self, item: FetchModelBusiness):
        self.url = item.url
    ...
    async def process(self, static_manager: dict[str, list] = {}):
        KobaLogger.info(f"⏳开始爬取: {self.url}, RERANKER: {self.reranker} ⏳")
        ...
        try:
            ...
            # context = await loop.run_in_executor(None, parser.parse)
        ...
        end_time = time.time()
        KobaLogger.info(f"⏳ {self.url} 检索总耗时:{(end_time - start_time)} 秒")
